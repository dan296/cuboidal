...
router.post("/shuffle", async (req: Request, res: Response) => {
  ...
  if (!Array.isArray(words)) return res.status(400).json({ error: "Invalid words array" });
...
router.get("/:key", async (req, res) => {
  ...
  if (!data) return res.status(404).json({ error: "Shuffle not found" });

  res.json({ words: JSON.parse(data) });
});
